...
export default function Home() {
  ...
  const handleDownloadSubtitles = async (languageCode: string) => {
    if (!videoInfo) return;
    ...
    try {
      const response = await fetch(
        `/api/subtitles/download?videoId=${videoInfo.videoId}&languageCode=${languageCode}`
      );
      const data = await response.json();

      if (!response.ok) {
        throw new Error(data.error || "자막을 다운로드하는데 실패했습니다");
      }
    ...
    <div className="min-h-screen bg-background p-4 sm:p-6 md:p-8">
      <div className="mx-auto max-w-4xl">
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {languages.map((lang) => (
                <button
                  ...
                  onClick={() => handleDownloadSubtitles(lang.languageCode)}
                  disabled={loading}
        ...
        {subtitles.length > 0 && (
          <div className="mb-6">
            <h2 className="mb-4 text-xl font-semibold">
              자막 내용 ({subtitles.length}개)
            </h2>
            <div className="max-h-[600px] overflow-y-auto rounded-lg border bg-card">
              <div className="divide-y">
                {subtitles.map((subtitle) => (
                  <div key={subtitle.index} className="p-4 hover:bg-accent/50">
        ...
        {subtitles.length > 0 && (
          <details className="mb-6">
            ...
            <pre className="mt-2 max-h-[400px] overflow-auto rounded-lg border bg-muted p-4 text-xs">
              {JSON.stringify({ videoInfo, languages, subtitles }, null, 2)}
            </pre>
          </details>
        )}
      </div>
